...
// Full-page form — mirrors UserForm.jsx style (um-form-page, gm-* inputs, um-* card)
// CREATE: uid=0, Pcode auto-generated by SP (not sent)
// EDIT  : load via getCustomerById, Pcode is READ ONLY
//
// Validations:
//   MANDATORY : Pname, Pbranch
//   FORMAT    : Mobile (10 digits), GSTNo (15 chars), Email (valid if provided), Pincode (6 digits)
//   Email is NOT mandatory — only validated if the user types something
...
export default function CustomerForm() {
  ...
  /* ── Validation ─────────────────────────────────────────────
     MANDATORY : Pname, Pbranch
     FORMAT    : Mobile (10 digits if provided)
                 GSTNo  (15 chars if provided)
                 email  (valid format if provided — NOT mandatory)
  ...
  /* ── Build clean payload — no spread to avoid key collisions ─
     All fields explicitly listed so there are zero duplicate keys.
     Mobile and Pincode converted to numbers only when non-empty.
  ── */
  const buildPayload = () => ({
    Active:        form.Active,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        // Don't send Pcode on create — SP auto-generates it
      ...
      // SP returns a business-rule error (duplicate, etc.) — show inline
      if (
      ...
      // Success — redirect to grid with toast
          ...
          {isEdit && !loaded ? (
            ...
            <>
              ...
              <form onSubmit={handleSubmit} noValidate autoComplete="off">
                ...
                {/* Country — always India, read only */}
                <Field label="Country">
                  <input
                    type="text"
                    className="gm-input"
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 20px" }}>
                  ...
                  {/* Email — NOT mandatory, validated only if filled */}
